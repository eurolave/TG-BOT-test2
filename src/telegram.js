...
export default class Bot {
  ...
  async onStart(msg) {
    const text = [
      'Привет! Я умею:',
      '• *Подбор по VIN* — `/vin WAUZZZ... [locale]`',
      '• *GPT-чат* — `/gpt <вопрос>` или просто сообщение',
      '• Сброс контекста GPT: `/reset`'
    ].join('\n');
    await this.bot.sendMessage(msg.chat.id, text, { parse_mode: 'MarkdownV2' });
  }
  async onHelp(m){ return this.onStart(m); }
  async onReset(m){ gptReset(m.chat.id); await this.bot.sendMessage(m.chat.id,'Контекст GPT очищен.'); }
  ...
  async handleVin(msg, vin, locale='ru_RU') {
    ...
    try {
      ...
      await this.bot.sendDocument(chatId, Buffer.from(JSON.stringify(json, null, 2)), {}, { filename: fileName, contentType: 'application/json' });
    } catch (e) {
  ...
  async handleGpt(msg, text) {
    const chatId = msg.chat.id;
    const typing = this.bot.sendChatAction(chatId, 'typing').catch(() => {});
    try {
      const answer = await gptChat(chatId, text || 'Привет!');
